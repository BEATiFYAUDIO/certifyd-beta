...
export function renderPublicInvite(invite: StaticInviteDto): string {
  ...
  const primaryAction = invite.startPath
    ? `<form method="post" action="${escapeAttribute(invite.acceptUrl)}"><input type="hidden" name="returnTo" value="${escapeAttribute(invite.acceptReturnPath)}"><button class="button primary" type="submit">Accept &amp; Start Mission</button></form>`
...
export function renderMissionStart(start: StaticMissionStartDto): string {
  ...
  const checklist = start.steps.length ? `
      <div class="mission-card">
        <p class="eyebrow">Mission checklist</p>
        <ol class="mission-steps">
          ${start.steps.map((step) => `<li>${escapeHtml(step)}</li>`).join('')}
        </ol>
      </div>
  ` : '';
  const guidanceHeading = start.continuationPath ? 'Agent ready?' : 'Next steps';
  ...
  ` : `
      <div class="mission-card">
        <p class="eyebrow">${escapeHtml(guidanceHeading)}</p>
