...
export default async function InvitesPage() {
  ...
  return <section className="panel"><h1>Invites</h1><table className="table"><thead><tr><th>Participant</th><th>Mission</th><th>Status</th><th>Code</th><th>Opened</th><th>Accepted</th><th>Open count</th><th>Published</th></tr></thead><tbody>{invites.map((invite) => <tr key={invite.id}><td><Link href={`/admin/participants/${invite.participantId}`}>{invite.participant.name}</Link></td><td>{invite.participantMission?.mission.name || '—'}</td><td><span className="badge">{invite.status}</span></td><td>{invite.codePreview}</td><td>{invite.openedAt?.toLocaleString() || '—'}</td><td>{invite.acceptedAt?.toLocaleString() || '—'}</td><td>{invite.openCount}</td><td>{invite.published ? <span className="badge good">Published</span> : <span className="badge">Local only</span>}</td></tr>)}</tbody></table></section>;
}
