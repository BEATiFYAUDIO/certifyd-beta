...
import { publicContactEmail } from '@/lib/static-publisher';
...
export default async function InvitePage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const invite = await prisma.invite.findFirst({ where: { code: parsed.data }, include: { participant: { include: { mission: true } } } });
  if (!invite || invite.status === InviteStatus.REVOKED || invite.status === InviteStatus.EXPIRED) return unavailable();
  const dto = buildStaticInviteDto({ ...invite, published: true }, publicContactEmail());
